...
const envPath = '/home/ec2-user/auto_auction/api/.env';
console.log('config.js: Loading .env from:', envPath);
dotenv.config({ path: envPath });
console.log('config.js: Loaded STRIPE_SECRET_KEY:', process.env.STRIPE_SECRET_KEY?.substring(0, 10) + '...');

const serviceAccountPath = process.env.FIREBASE_SERVICE_ACCOUNT_PATH || '/home/ec2-user/auto_auction/api/auto-auction-ce674-firebase-adminsdk-fbsvc-7aa3c3be76.json';
console.log('config.js: Firebase service account path:', serviceAccountPath);

let serviceAccount: admin.ServiceAccount;

try {
  serviceAccount = require(serviceAccountPath);
  console.log('config.js: Loaded Firebase service account:', serviceAccount.projectId);
} catch (error) {
  console.error('config.js: Failed to load Firebase service account:', error);
...
if (!admin.apps.length) {
  ...
  console.log('config.js: Firebase Admin SDK initialized');
}
